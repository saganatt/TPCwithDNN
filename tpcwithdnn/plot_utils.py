...
def plot_distortion(config, h_dist, h_deltas, h_deltas_vs_dist, prof, suffix, opt_name):
    ...
    h_dist.Draw("colz")
    txt1 = add_desc_to_canvas(config, 0.18, 0.7, 0.3, 0.9, 0.04, True, False, True, False)
    txt1.Draw()
    c2 = cev.cd(2)
    ...
    txt2 = add_desc_to_canvas(config, 0.18, 0.7, 0.3, 0.9, 0.04, True, False, True, False)
    txt2.Draw()
    c3 = cev.cd(3)
    ...
    h_deltas.Draw()
    txt3 = add_desc_to_canvas(config, 0.18, 0.7, 0.3, 0.9, 0.04, True, False, True, False)
    txt3.Draw()
    c4 = cev.cd(4)
    ...
    prof.Draw()
    txt4 = add_desc_to_canvas(config, 0.45, 0.7, 0.85, 0.9, 0.04, True, False, True, False)
    txt4.Draw()
    #cev.cd(5)
...
def add_desc_to_canvas(config, xmin, ymin, xmax, ymax, size,
...
def draw_multievent_hist(config, events_counts, func_label, hist_name, source_hist):
    ...
    for iname, opt in enumerate(config.opt_predout):
        if opt == 1:
            opt_name = config.nameopt_predout[iname]
            ...
            for i, (train_events, _, _, _) in enumerate(events_counts):
                filename = "%s/output_%s_nEv%d.root" % (config.dirval, config.suffix,
                                                        train_events)
